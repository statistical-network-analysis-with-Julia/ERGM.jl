...
# The value of a numeric vertex attribute for vertex `v`, for the raw
# (un-materialized) NodeCov/AbsDiff statistics. There is deliberately NO
# zero-fill default: statnet refuses NA attribute values, `ERGMModel` refuses
# a formula whose attribute is not set on every vertex (see
# `_validate_formula`), and a direct `compute(NodeCov(:x), net)` on
# incomplete data must not quietly return a number either.
@inline function _vertex_value(term, attr_vals, v)
    val = get(attr_vals, v, nothing)
    (val === nothing || val === missing) && _throw_no_vertex_value(term, v)
    return val
end

@noinline _throw_no_vertex_value(term, v) = throw(ArgumentError(
    "term '$(name(term))' needs vertex attribute :$(term.attr) on every vertex, " *
    "but vertex $v has no value. statnet refuses NA attribute values; set a " *
    "value for every vertex or drop the term."))
...
rich = Dict(v => (w > 40 ? "rich" : "poor") for (v, w) in get_vertex_attribute(net, :wealth))
set_vertex_attribute!(net, :class, rich)
fit = fit_ergm(net, [Edges(), NodeFactor(:class)])
fit.model.formula.terms.names      # ["edges", "nodefactor.class.rich"] — "poor" is the reference
compute(NodeFactor(:class; level="rich"), net)   # 20.0 endpoint appearances
```
"""
...
"""
    NodeCov <: NodalTerm
    NodeCov(attr; transform=:none)

Main effect of a continuous vertex attribute (R ergm's `nodecov`): the sum
of the attribute values of both endpoints over the edges, named
`"nodecov.<attr>"`.

The attribute must be set on **every** vertex: as in statnet (which refuses
NA attribute values), `ERGMModel` throws an `ArgumentError` naming the term,
the attribute and the vertices without a value, and a direct
`compute`/`change_stat` on incomplete data throws rather than zero-filling.

# Fields
- `attr::Symbol`: Vertex attribute name
- `transform::Symbol`: Optional transform of each vertex's value before
  summing — `:none` (default), `:log` (`log(x + 1)`) or `:sqrt`. Any other
  symbol is an `ArgumentError` (it used to fall back silently to `:none`).
...
name(NodeCov(:wealth; transform=:log))     # "nodecov.wealth"
```
"""
struct NodeCov <: NodalTerm
    ...
    function NodeCov(attr::Symbol; transform::Symbol=:none)
        transform in (:none, :log, :sqrt) || throw(ArgumentError(
            "NodeCov transform must be :none, :log or :sqrt (got :$transform)"))
        return new(attr, transform)
    end
end
...
function compute(term::NodeCov, net)
    ...
    for e in edges(net)
        ...
        val_i = _vertex_value(term, attr_vals, i)
        val_j = _vertex_value(term, attr_vals, j)
...
function change_stat(term::NodeCov, net, i::Int, j::Int)
    ...
    val_i = _transform_value(_vertex_value(term, attr_vals, i), term.transform)
    val_j = _transform_value(_vertex_value(term, attr_vals, j), term.transform)
...
rich = Dict(v => (w > 40 ? "rich" : "poor") for (v, w) in get_vertex_attribute(net, :wealth))
set_vertex_attribute!(net, :class, rich)
compute(NodeMatch(:class), net)                          # 10.0 within-class ties
levels = sort(unique(values(rich)))                      # ["poor", "rich"]
terms = [NodeMatch(:class; diff=true, level=l) for l in levels]
sum(compute(t, net) for t in terms)                      # 10.0
```
"""
...
rich = Dict(v => (w > 40 ? "rich" : "poor") for (v, w) in get_vertex_attribute(net, :wealth))
set_vertex_attribute!(net, :class, rich)
compute(NodeMismatch(:class), net) + compute(NodeMatch(:class), net) == 20.0   # true
```
"""
...
Absolute difference in a continuous attribute between edge endpoints (R
ergm's `absdiff`): `Σ_edges |xᵢ − xⱼ|^pow`, named `"absdiff.<attr>"` for
`pow = 1` and `"absdiff<pow>.<attr>"` otherwise (`absdiff2.wealth`,
`absdiff0.5.wealth` — R's labels, so two powers of one attribute in a
formula print distinct names; pinned by the provenanced fixture).

The attribute must be set on **every** vertex: as in statnet, `ERGMModel`
throws an `ArgumentError` naming the vertices without a value, and a direct
`compute`/`change_stat` on incomplete data throws rather than zero-filling.

# Fields
- `attr::Symbol`: Vertex attribute name
- `pow::Float64`: Power to raise the difference to (default 1; any `Real`
  is accepted and stored as a `Float64`, so R's `absdiff("wealth", pow=2)`
  is `AbsDiff(:wealth; pow=2)`)

# Example
```julia
using ERGM
net = load_dataset(:florentine_marriage)
compute(AbsDiff(:wealth), net)     # 1146.0
compute(AbsDiff(:wealth; pow=2), net)   # 91570.0 (R: summary(flomarriage ~ absdiff("wealth", pow=2)))
name(AbsDiff(:wealth; pow=2))      # "absdiff2.wealth"
```
"""
struct AbsDiff <: NodalTerm
    ...
    AbsDiff(attr::Symbol; pow::Real=1.0) = new(attr, Float64(pow))
end

name(term::AbsDiff) =
    term.pow == 1 ? "absdiff.$(term.attr)" : "absdiff$(_decay_label(term.pow)).$(term.attr)"
...
function compute(term::AbsDiff, net)
    ...
    for e in edges(net)
        i, j = Int(src(e)), Int(dst(e))
        val_i = Float64(_vertex_value(term, attr_vals, i))
        val_j = Float64(_vertex_value(term, attr_vals, j))
...
rich = Dict(v => (w > 40 ? "rich" : "poor") for (v, w) in get_vertex_attribute(net, :wealth))
set_vertex_attribute!(net, :class, rich)
fit = fit_ergm(net, [Edges(), NodeMix(:class)])
fit.model.formula.terms.names   # ["edges", "mix.class.poor.rich", "mix.class.rich.rich"]
compute(NodeMix(:class, "poor", "rich"), net)   # 10.0 cross-class ties
```
"""
