...
# The Bernoulli model (flo ~ edges): its MLE is logit(density) in closed form,
# so it is the one fit an MCMLE of a dyad-independent model can be checked
# against without a second Monte-Carlo run. Exact on both sides.
f_eo <- flo ~ edges
fit_eo <- quiet_ergm(f_eo)

# Extra deterministic statistics the Julia testsuite once carried as literals
sum_extra <- summary(flo ~ triangle)

# gof() OBSERVED panels (round 3, 2026-09): deterministic functions of the
# observed graph. The distance panel counts UNORDERED pairs on an undirected
# network and ends with the unreachable pairs under the name "Inf" -- ERGM.jl's
# panel used to count ordered pairs (twice R) and to drop the Inf row.
gof_di <- suppressMessages(gof(fit_di, GOF = ~degree + esp + distance,
...
cat("# So are gof()'s observed degree / esp / geodesic-distance panels.\n")
cat("gof_degree = 1e-9\n")
cat("gof_esp = 1e-9\n")
cat("gof_distance = 1e-9\n")
cat("#\n")
...
cat("# ERGM.jl Newton-Raphson to 1e-8. 1e-6 is well inside both, and is ~1e-4 of the\n")
cat("# smallest standard error in the model. If this fails it is a BUG -- do not\n")
cat("# loosen it.\n")
cat("di_coefficients = 1e-6\n")
cat("di_std_errors = 1e-6\n")
cat("# The edges-only (Bernoulli) MLE is logit(density) on both sides: same\n")
cat("# argument, same tolerance.\n")
cat("eo_coefficients = 1e-6\n")
cat("eo_std_errors = 1e-6\n")
cat("#\n")
...
cat(sprintf("summary_statistic_names = [%s]\n",
            strs(c(names(sum_di), setdiff(names(sum_dd), names(sum_di)), names(sum_extra)))))
cat(sprintf("summary_statistics = [%s]\n",
            num(c(sum_di, sum_dd[setdiff(names(sum_dd), names(sum_di))], sum_extra))))
cat("\n# --- gof() observed panels of the dyad-independent fit: degree over 0:(n-1),\n")
cat("# esp over 0:(n-2), distance over UNORDERED pairs with the unreachable\n")
cat("# pairs last under the label \"Inf\" (15 of the 120 dyads: Pucci is isolated)\n")
cat(sprintf("gof_degree = [%s]\n", num(gof_di$obs.deg)))
cat(sprintf("gof_esp = [%s]\n", num(gof_di$obs.esp)))
cat(sprintf("gof_distance_labels = [%s]\n", strs(names(gof_di$obs.dist))))
cat(sprintf("gof_distance = [%s]\n", num(gof_di$obs.dist)))
...
cat("\n# --- (a\') Bernoulli model (edges only): MLE = logit(density), exact ------\n")
cat(sprintf("eo_coefficients = [%s]\n", num(coef(fit_eo))))
cat(sprintf("eo_std_errors = [%s]\n", num(se_of(fit_eo))))
